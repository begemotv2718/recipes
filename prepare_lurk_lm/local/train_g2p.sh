...
cmudict_plain=$g2p_dir/cmudict_plain
cmudict_clean=$g2p_dir/cmudict_clean

sequitur=$KALDI_ROOT/tools/sequitur/bin/g2p.py
export PATH=$PATH:$KALDI_ROOT/tools/sequitur/bin
export PYTHONPATH=$PYTHONPATH:`readlink -f $KALDI_ROOT/tools/sequitur/lib/python*/site-packages`

if [ $stage -le 2 ]; then
  echo "Removing the pronunciation variant markers ..."
  grep -v ';;;' $cmudict_dir/ru.dic | \
    perl -ane 'if(!m:^;;;:){ s:(\S+)\(\d+\) :$1 :; print; }' \
    > $cmudict_plain || exit 1;
  echo "Removing special pronunciations(not helpful for G2P modelling)..."
  cp $cmudict_plain $cmudict_clean
fi
